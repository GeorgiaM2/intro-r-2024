...
df <- df |> bind_rows(slice_tail(df)) # take last row and add to end of df (duplicating the row that was chosen)
df
tail(df)

df <- df |> distinct() # this returned the the non repeating rows per set of values 
tail(df)
...
df_travel$travel_speed <- (df$travel_distance /
  df_travel$travel_time * 60) # mph
df_travel

df_travel <- df_travel |>
  mutate(travel_speed = travel_distance / travel_time *60 ) # what you should do instead 
summary(df_travel)


# if just renaming 
df_travel <- df_travel |>
  rename(travel_mph = travel_speed) # travel_moh is new name with values from travel _speed
colnames(df_travel)
df_travel
...
df_travel <- df_travel |>
  mutate(slow_trip =
           case_when(
             ...
             .default = 0 # ALL ALSE or NA would be 0
           ))
df_travel
